...
// ------------------------------------------
//  Waveshare 2.13 inch (D) flexible display
// ------------------------------------------
...
static const unsigned int EPD_2IN13D_WIDTH = 104;
...
void WaveshareEPaper2P13InD::initialize() {
  ...
  this->data(0x0e);     //VCOM to 0V fast

  this->command(0x30);	//PLL setting
  this->data(0x3a);     // 3a 100HZ   29 150Hz 39 200HZ	31 171HZ

  this->command(0x61);	//resolution setting
  this->data(EPD_2IN13D_WIDTH);
  this->data((EPD_2IN13D_HEIGHT >> 8) & 0xff);
  this->data(EPD_2IN13D_HEIGHT& 0xff);

  this->command(0x82);	//vcom_DC setting
  this->data(0x28);
}

void WaveshareEPaper2P13InD::set_full_reg_() {
    this->command(0x82);
    this->data(0x00);	
    this->command(0X50);
    this->data(0x97);
    ...
    for(count=0; count<44; count++) {
        this->data(EPD_2IN13D_lut_vcomDC[count]);
    }

    this->command(0x21);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_ww[count]);
    }

    this->command(0x22);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_bw[count]);
    }

    this->command(0x23);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_wb[count]);
    }

    this->command(0x24);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_bb[count]);
    }
}

void WaveshareEPaper2P13InD::set_part_reg_() {
    this->command(0x82);			//vcom_DC setting
    this->data(0x03);
    this->command(0X50);
    this->data(0x47);
    ...
    this->command(0x20);
    for(count=0; count<44; count++) {
        this->data(EPD_2IN13D_lut_vcom1[count]);
    }

    this->command(0x21);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_ww1[count]);
    }

    this->command(0x22);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_bw1[count]);
    }

    this->command(0x23);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_wb1[count]);
    }

    this->command(0x24);
    for(count=0; count<42; count++) {
        this->data(EPD_2IN13D_lut_bb1[count]);
    }
}

void WaveshareEPaper2P13InD::turn_on_display_() {
  this->command(0x12);
  delay(10);  // NOLINT
  this->wait_until_idle_();
}
...
int WaveshareEPaper2P13InD::get_height_internal() { return EPD_2IN13D_HEIGHT; }
